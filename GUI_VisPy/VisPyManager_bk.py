...
def update_VisCam(visView, imgDimZYX):
    [sz, sy, sx] = imgDimZYX
    print()
    print('update_VisCam()')
    ...
    if sz>1:        
        visView.camera.set_range(x=[0, sx], y=[0, sy], z=[0, sz], margin=0.5)
        visView.camera.center = (sx//2, sy//2, sz//2)
    else:
        visView.camera.set_range(x=[0, sx], y=[0, sy], z=[0, sz], margin=0.5)
        visView.camera.center = (sx//2, sy//2)
...
def add_visCubeObjectToView(visView):
    visCube = scene.visuals.Cube(color=None, edge_color=(1, 1, 1, 0.1))
    return visCube
...
def update_visDots(visView, visDots, v_p, R=5, dpi=90, isText=True, c=[0, 1, 0, 0.1]):
    visDots.parent = None
...
def update_visDisplay(visView, visCam, visVol, visBox, visAxes, img3D, mode='Scroll'):  
...
def update_visOrthoView(img3D, visOrtho_XYZ, visOrtho_XY, visOrtho_YZ, visOrtho_XZ, mode='Start'):
    ...
    update_visDisplay(visView, visCam, visVol, visBox, visAxes, img2D_XY)
    update_visAxes(visView, visAxes, scale=imgDimXYZ)
    
    # VisPlot: 2D YZ  
    visView, visCam, visVol, visBox, visAxes = visOrtho_YZ 
    update_visDisplay(visView, visCam, visVol, visBox, visAxes, img2D_YZ)
    ...
    update_visDisplay(visView, visCam, visVol, visBox, visAxes, img2D_XZ)
    Mr_Y     = np.matrix([[1,  0,  0],
                          [0,  0, +1],
                          [0, -1,  0]])
    Mr = Mr_Y
...
def update_visPlane(img2D, visOrtho, mode='Scroll'):    
    # VisPlot: 2D XY
